...
public class SandboxArreglos
{
    ...
    {
    	int[] nuevoArreglo = new int[arregloEnteros.length + 1];
        posicion = Math.max(posicion, 0);
        posicion = Math.min(posicion, arregloEnteros.length);
    ...
    public boolean mismosEnteros( int[] otroArreglo )
    {
        ...
        for (int entero : otroArreglo) {
            if (!histogramaOriginal.containsKey(entero) || histogramaOriginal.get(entero) == 0) {
                return false;
            }
            histogramaOriginal.put(entero, histogramaOriginal.get(entero) - 1);
        }
        return true;
    }
