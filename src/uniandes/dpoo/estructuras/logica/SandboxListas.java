...
public class SandboxListas
{
    ...
    {
    	List<Integer> nuevosEnteros = new ArrayList<>();
        for (Integer entero : listaEnteros) {
            nuevosEnteros.add(Math.abs(entero));
        }
        listaEnteros = nuevosEnteros;
    }
    ...
    {
    	listaEnteros.sort(null); // Orden natural (menor a mayor)
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
        ...
        for (int i = 0; i < cantidad; i++) {
            int randomValue = minimo + (int) (Math.random() * ((maximo - minimo) + 1));
